...
public class c6_CombiningPublishers extends CombiningPublishersBase {
    ...
    public void mail_box_switcher() {
        ...
        Flux<Message> myMail = mailBoxPrimary().switchOnFirst((signal, messageFlux) -> {
            ...
            if (message != null && message.metaData.equals("spam")) {
    ...
    public void prettify() {
        ...
        Mono<Boolean> successful = Mono.when(openFile()).and(writeToFile("0x3522285912341")).then(closeFile()).thenReturn(true);
...
//        return Mono.empty(); //otherwise, return empty
    ...
    public void cleanup() {
        ...
        //todo: feel free to change code as you need
        Flux<String> stream = Flux.usingWhen(StreamingConnection.startStreaming(), stringFlux -> stringFlux, stringFlux -> StreamingConnection.closeConnection());
